...
# Pi Node Monitor Live v2 - READ ONLY
# STANDARD: PiNodeMonitorLive_CMD_v2_REBUILT (user package)
# Priority: Stellar/Docker (primary) -> Windows hardware -> Pi Desktop optional fallback
# No restart/stop/start/delete/config changes.
#
# Integration only:
# - Paths write to App Data\PiNodeMonitorLive\ and Data\node_history.json
# - Temp DLL: assets\OpenHardwareMonitorLib.dll OR Data\OpenHardwareMonitorLib.dll
# - ASCII dashboard (avoid Windows encoding crash)
# - History save every Node cycle (60s)
...
while ($true) {
    ...
    if (($now - $lastPorts).TotalSeconds -ge 60) {
        ...
        if ($d.Status) { $State.ContainerStatus = $d.Status }
        $lastPorts = $now
    }
    ...
    $ledgerStr = if ($null -eq $State.Ledger) { "-" } else { $State.Ledger }
    $ageStr = if ($null -eq $State.Age) { "-" } else { $State.Age }
    $inStr = if ($null -eq $State.In) { "-" } else { $State.In }
    $outStr = if ($null -eq $State.Out) { "-" } else { $State.Out }
    $cpuStr = if ($null -eq $State.CPU) { "N/A" } else { $State.CPU }
    $ramStr = if ($null -eq $State.RAM) { "N/A" } else { $State.RAM }
    $tempStr = if ($null -eq $State.Temp) { "N/A" } else { $State.Temp }
    $diskStr = if ($null -eq $State.Disk) { "N/A" } else { $State.Disk }
    $vmmStr = if ($null -eq $State.Vmmem) { "N/A" } else { $State.Vmmem }
    $nc = if ($State.Node -eq "Synced!") { "Green" } elseif ($State.Node -eq "Unavailable") { "DarkYellow" } else { "Yellow" }
    $ac = if ($State.Anomaly -eq "Normal") { "Green" } else { "Yellow" }

    Clear-Host
    Write-Host "================================================================" -ForegroundColor DarkCyan
    Write-Host "         PI NODE MONITOR LIVE v2  |  READ ONLY                 " -ForegroundColor Cyan
    Write-Host "================================================================" -ForegroundColor DarkCyan
    Write-Host ("  NODE   {0,-16}  LEDGER {1,-12}  AGE {2}s" -f $State.Node, $ledgerStr, $ageStr) -ForegroundColor $nc
    Write-Host ("  PEERS  IN {0,-5} OUT {1,-5}   SOURCE: {2}" -f $inStr, $outStr, $State.Source)
    Write-Host ("  CPU    {0}%    RAM {1}%    TEMP {2} C" -f $cpuStr, $ramStr, $tempStr)
    Write-Host ("  DISK   {0}%    VMMEM {1} GB" -f $diskStr, $vmmStr)
    Write-Host ("  PORTS  {0}" -f $State.Ports)
